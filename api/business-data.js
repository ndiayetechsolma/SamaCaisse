...
const [entrepriseResult, magasinsResult, produitsResult, ventesResult, depensesResult, caissesResult, personnelResult] = await Promise.all([
    ...
    client.from('produits').select('*, magasins(nom)').eq('compte_id', compteId).eq('entreprise_id', entrepriseId).order('nom'),
  ...
  if (failing) return response.status(500).json({ error: failing.error.message });
